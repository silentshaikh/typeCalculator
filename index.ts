...
if (ourCalculator.Operator === "Addition") {
  console.log(
    `The Addition is : ${
      ourCalculator.FirstNumber + ourCalculator.SecondNumber
    }`
  );
  ...
  if(ourCalculator.SecondNumber === 2){
    console.log(
      `The Square of ${ourCalculator.FirstNumber} is : ${
        ourCalculator.FirstNumber ** ourCalculator.SecondNumber
      }`
      );
  }else{
  ...
  console.log(
    `The Sin of ${anglValue} degree is : ${
      Math.sin((anglValue*Math.PI)/180)
    }`
  );
}
 else {
